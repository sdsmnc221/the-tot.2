...
export async function getSkyTickets(limit = 50): Promise<SkyTicketEntry[]> {
  try {
    ...
    console.log("Retrieved from Redis:", ticketsArray.length, "sky tickets");
...
export async function saveSkyTicket(
  ...
  try {
    ...
    console.log("Current sky tickets before save:", tickets.length);
    ...
    console.log("Sky ticket saved to Redis. Total now:", tickets.length);
    console.log("New ticket number:", ticketData.ticketNumber);
    return ticketData;
  } catch (error) {
...
export async function updateSkyTicket(
  ...
  try {
    console.log("Updating sky ticket:", ticketNumber, "with:", updates);

    // Get the existing ticket
    const existingTicket = await getSkyTicketById(ticketNumber);
    if (!existingTicket) {
      console.log("Ticket not found for update:", ticketNumber);
      return null;
    }
    ...
    console.log("Sky ticket updated successfully:", ticketNumber);
...
export async function getSkyTicketStats() {
  try {
    ...
    const keys = await redis.keys("sky-ticket:*");
